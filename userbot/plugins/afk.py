...
async def _(event):
    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await event.edit(f"miss sky Is Going Afk ! And The Reason is {reason}")
        else:
            await event.edit(f"miss sky  is Going")
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        message_to_reply = f"**My Boss is AFK** ! \n\n**Reason** : `{reason}` \n\n**Afk Since** : {afk_since}" + \
            ...
            if reason \
            else f"**Hello, miss sky AFK Right Now And May Be Forgot List Reason ! Any Way she Will Come Back Soon !**"
